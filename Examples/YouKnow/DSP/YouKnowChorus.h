...
public:
    ...
    // Once an Off wet-mute has settled exactly to zero, advances only the
    // free-running LFO, writes the bit-exact dry result, marks the wet path for
    // a from-silence rebuild, and returns true. Returns false while process()
    // must still run the complete path.
    ...
    // C3, the integrator capacitor. The original instrument's own page prints it in Roland's
    ...
    // original-chorus clone (0.537/0.879 Hz) land within 3%, and the JUNO-60's own
    ...
    // 6.4352941 MOhm over 3.9638889 MOhm. This ratio is the original instrument's own,
...
private:
    ...
    // not a original instrument measurement.  The optional amplitudes therefore default
