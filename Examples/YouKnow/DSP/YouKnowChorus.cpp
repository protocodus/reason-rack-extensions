...
// Frozen from the source builder as exact hexadecimal doubles: this remains
// the same optimized curve while avoiding a runtime-initialized Rack global.
constexpr std::array<BbdTransferHermiteNode,
                     bbdTransferHermiteIntervals + 1u>
    bbdTransferHermiteTable {{
#include "YouKnowBbdTransferTable.inc"
}};
...
// ground. The former ideal-source, one-leg solve put this at 23.461 kHz.
// Panasonic's MN3009 Gi-RL typical curve instead supports a finite local
// Thevenin estimate: reading its slope around the installed 50--100 kOhm
// region gives about 3.7 kOhm per follower. The datasheet application circuit
// supports reading RL as separate loads on OUT1 and OUT2 before their 10 kOhm
// balance pot. Both follower outputs remain electrically present, so the two
// (3.3 + 3.7) kOhm paths are in parallel. In parallel with 47 kOhm, they give
// the isolated tap node a 22.209 kHz Thevenin corner. R98/R107 then loads that
// node, so the realised circuit below solves the coupled modes rather than
// treating 22.209 kHz as a separate pole. The Roland common-mode load is about
// 97.3 kOhm per pin, almost Panasonic's 100 kOhm fixture, so no separate
// insertion-gain guess is added. Curve-read uncertainty and a wet-only
// original-unit sweep remain OQ-04.
//
...
// original instrument's own p. 15 scan reads them so at designator level (2026-08-07):
    ...
    ChorusMode mode, bool useA11EffectiveTimingProfile) noexcept
{
    ...
    // original-chorus clone corroborate both within 3% (0.537/0.879 Hz), and both
    ...
    //
    // The sweep endpoints are the original instrument's own third-party-measured figures:
    ...
    // directly against a real original instrument by its owner, who published the scope
                     ...
                     bool useA11EffectiveTimingProfile) noexcept
{
...
#endif
    // A settled bypass leaves stale muted-line history. Rebuild from silence;
    // the existing from-zero wet glide then brings an engaged effect in.
    ...
    // a ~50-point click-timing series across the original instrument's modulation cycle fits
