...
// Published ordinals are stored by song state and must not move. PolyZoned is
// appended so an existing Fast song keeps the table-Hermite kernel bit for
// bit; the polynomial rung replaces the fine-table lookup for |x| < 1 and
// batches the four independent stage nonlinearities per RHS evaluation.
...
struct EngineParameters
{
    ...
    // Off by default: the only trajectory measurement in existence (comparison instrument's
    // ~50-point click-timing series, 16 us RMS residual against a straight
    // line) reads the original instrument's delay as linear in time, so the linear sweep
    ...
    [[nodiscard]] bool operator==(const EngineParameters& other) const noexcept
    {
...
public:
    ...
    // The panel's +/-50 cents retains B-2's signed byte in 1/256-semitone
    // units: +127 (+49.609375 cents) versus -128. Reason's combined global
    // and panel tuning extends to +/-150 cents by adding a rounded remainder
    // outside that hardware segment; malformed input maps to zero.
    ...
    {
        static constexpr int trimSteps = 128;
        static constexpr float trimCeiling = 8.0f;
        ...
        // reading of the original instrument's own module, and it is what ships.
    ...
    // this instrument about 19 dB quieter than a peer original instrument emulation
    // measured on the same patch and notes (comparison instrument at its own default:
    // -17.5 dBFS RMS; this engine at maximum volume: -36.2). A real original instrument only
    ...
    // original instrument voltage (see internalVoltsPerUnit), so this is a product
    ...
    //
    //   VCA CV (0..+10 V from the S/H) -> the 10 kOhm input resistor 10k -> node (C58 0.1 uF to
    ...
    //
    //   Ie * R + Vt * ln(1 + Ie / Is) = V_cv,   R = the 10 kOhm input resistor + R105 = 32 kOhm
    //
    ...
    struct VoiceVcaSignalLaw
    {
        ...
        static constexpr float holdStandoffVolts = 0.26f;
        // the 10 kOhm input resistor 10k + R105 22k (p. 13).
...
private:
    ...
    // on Ultramaster comparison instrument and +6.87 on Arturia's Jun-6 V against this
    ...
    // sat low. 7.57 follows comparison instrument, the one of the two that models the original instrument
    ...
    // correction, and comparison instrument's own pulse reading is an outlier against both
