...
namespace youknow
{
// The source reads outputJackPi; the Rack target is C++17, so the same
// double literal is spelled here.
constexpr double outputJackPi = 3.14159265358979323846;
...
struct OutputJackLowPass
{
    ...
    {
        const double wm = std::min(0.9 * outputJackPi,
                                   2.0 * outputJackPi * 20000.0 / sampleRate);
        const double beta = 2.0 / (wm * wm) - 1.0 / (1.0 - std::cos(wm));
        const double wc = 2.0 * outputJackPi * cornerHz / sampleRate;
