...
- [ ] Preserve identity, saved controls, automation IDs, sockets and latency.
- [ ] Pass strict engine, frozen-table, envelope-firmware and 19-group DSP regression contracts.
- [ ] Compare all 42 scalar scenarios bit-for-bit with upstream, including the Rack product configuration.
- [ ] Pass wrapper contracts under ASan/UBSan, including musical note phrases and malformed controls.
- [ ] Fuzz engine and wrapper with exact press-count models, optimized and sanitized.
- [ ] Pass final calibrated-bank renders at Aging 0% and 50%.
- [ ] Complete low/high six-note stress renders.
- [ ] Complete automation artifact contract on the product configuration.
- [ ] Record native default-quality timing without concurrent task load; retain its failure.
- [ ] Qualify wall-clock deadlines and representative Reason playback performance.
- [ ] Regenerate the panels for the f22 rear version label: run the Panels
  workflow by hand on the branch (it renders on a macOS runner and commits the
  result), or `python3 Design/render_panels.py` on a Mac.
...
- [ ] Build local45 Deployment and inspect the installed payload; this also confirms the
  ...
  does not read.
- [ ] Pass source CI on the committed candidate.
